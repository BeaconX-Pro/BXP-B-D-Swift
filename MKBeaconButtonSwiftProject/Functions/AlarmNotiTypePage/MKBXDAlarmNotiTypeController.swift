//
//  MKBXDAlarmNotiTypeController.swift
...
public enum MKBXDAlarmNotiTypeControllerType: Int {
    case single = 0
    case double = 1
    case long = 2
    case abnormal = 3
...
public final class MKBXDAlarmNotiTypeController: MKSwiftBaseViewController {

    public var pageType: MKBXDAlarmNotiTypeControllerType = .single
    ...
    private lazy var headerView: MKBXDNotificationTypePickerView = {
        let view = MKBXDNotificationTypePickerView(frame: CGRect(x: 0, y: 0, width: MKScreen.width, height: 100))
        view.delegate = self
    ...
    private lazy var headerViewModel: MKBXDNotificationTypePickerViewModel = {
        ...
        m.needButton = false
        m.typeLabelMsg = "Alarm notification type"
    ...
    private lazy var dataModel: MKBXDAlarmNotiTypeModel = {
        let m = MKBXDAlarmNotiTypeModel()
    ...
    private func readDataFromDevice() {
        ...
        dataModel.readData(sucBlock: { [weak self] in
            ...
            self?.headerView.updateNotificationType(self?.dataModel.alarmNotiType ?? 0)
    ...
    private func numberOfRows(section: Int) -> Int {
        if section == 0 || section == 1 {
            var need = false
            if dataModel.alarmNotiType == 1 { need = true }
            else if isCR && (dataModel.alarmNotiType == 4 || dataModel.alarmNotiType == 5) { need = true }
            else if !isCR && dataModel.alarmNotiType == 3 { need = true }
            if section == 0 { return need ? section0List.count : 0 }
            return need ? section1List.count : 0
        }
        if section == 2 || section == 3 {
            var need = false
            if isCR && (dataModel.alarmNotiType == 2 || dataModel.alarmNotiType == 4) { need = true }
            if section == 2 { return need ? section2List.count : 0 }
            return need ? section3List.count : 0
        }
        if section == 4 || section == 5 {
            var need = false
            if isCR && (dataModel.alarmNotiType == 3 || dataModel.alarmNotiType == 5) { need = true }
            else if !isCR && (dataModel.alarmNotiType == 2 || dataModel.alarmNotiType == 3) { need = true }
    ...
    private func loadSubViews() {
        defaultTitle = "Alarm notification type"
...
extension MKBXDAlarmNotiTypeController: UITableViewDelegate, UITableViewDataSource {
    ...
    /// ✅ 参考 MKBXSSensorAdvancedController：每个 section 都返回 10
    /// 当返回 0 时，iOS 15+ 的 sectionHeaderTopPadding 可能引发 layout bug
    public func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        return heightForHeader(section: section)
    }

    private func heightForHeader(section: Int) -> CGFloat {
        // 与 numberOfRows 判断一致：只有该 section 有内容时，header 才占位
        if section == 0 || section == 1 {
            var need = false
            if dataModel.alarmNotiType == 1 { need = true }
            else if isCR && (dataModel.alarmNotiType == 4 || dataModel.alarmNotiType == 5) { need = true }
...
extension MKBXDAlarmNotiTypeController: MKSwiftTextFieldCellDelegate {
...
extension MKBXDAlarmNotiTypeController: MKBXDNotificationTypePickerViewDelegate {

    public func bxd_notiTypePickerViewTypeChanged(_ type: Int) {
        dataModel.alarmNotiType = type
        tableView.reloadData()
    }
